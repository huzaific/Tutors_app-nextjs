const PurchasingPointVoucherStudent = () => {
    return ( 
        <div class="content">
        <section class="faq-wrap purchasing-point-voucher-wrap">
            <div class="container">
                <div class="ask-question">
                    <h5>프리미엄 이용권 구매하기</h5>
                </div>
                <div class="point-held-wrap">
                    <div class="point-held">
                        <h6><span>P</span>보유포인트</h6>
                        <p>6,000<span>Point</span></p>
                    </div>
                    <div class="student-premium-member">
                        <ul>
                            <li><span>[학생 닉네임 표시부분]</span><b> 학생은 일반 회원입니다.</b></li>
                        </ul>
                        <a href="point-recharge2.html" target="_self" class="general-btn green-bg">포인트 충전</a>
                    </div>
                </div>
                <a href="#" class="purchase-pass-btn general-btn green-bg">이용권 구매하기</a>
                <div class="premium-voucher-purchase-wrap">
                    <div class="premium-voucher-purchase">
                        <h3>프리미엄 이용권 구매 혜택</h3>
                        <div class="premium-teacher-list-wrap">
                            <div class="premium-teacher-list">
                                <div class="premium-package-wrap">
                                    <h2>20,000</h2>
                                    <div class="time-limt-point">
                                        <h4>Point</h4>
                                        <span>1개월 /30일</span>
                                    </div>
                                </div>
                                <ul>
                                  <li>- 질문게시판 1개월 무제한 질문</li>
                                  <li>- 화상과외 광고 프리패스</li>
                                </ul>
                            </div>
                        </div>
                        <a href="#" data-toggle="modal" data-target="#payment-alert-modal" class="general-btn green-bg long-btn">구매하기</a>
                    </div>
                    <p>※ 이용권 사용기간 : 구매한 날짜로부터 1개월</p>
                </div>
            </div>
        </section>
    </div>
  
     );
}
 
export default PurchasingPointVoucherStudent;
       